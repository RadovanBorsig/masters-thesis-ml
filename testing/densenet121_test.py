...
model.classifier = nn.Sequential(nn.Dropout(p=0.5), nn.Linear(model.classifier.in_features, 4))
...
fig.text(0.5, 1.01, f"DenseNet-121  ·  Testovacia úspešnosť: {test_acc:.2f}%", ha='center', fontsize=11, fontweight='bold', color='#1a1a1a', fontfamily='monospace')

plt.tight_layout()
plt.savefig("denseNet121_confusion_matrix.png", dpi=300, bbox_inches='tight', pad_inches=0.3)
plt.show()
...
plt.tight_layout()
plt.savefig("denseNet121_roc_curve.png", dpi=300, bbox_inches='tight', pad_inches=0.3)
plt.show()
