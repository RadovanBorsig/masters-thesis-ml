...
def save_pair(img_path, true_label, idx, fname):
    ...
    ax.axis('off')
    ax.set_title(f'{TARGET_CLASS} {idx}', fontsize=10, fontweight='bold', pad=4)
    ...
    ax.imshow(vis)
    ax.axis('off')
    ax.set_title(f'ConvNeXt  Pred: {pred_name} ({confidence * 100:.1f}%) {correct}', fontsize=10, fontweight='bold', color=color, pad=6)
    plt.tight_layout(pad=0.3)
