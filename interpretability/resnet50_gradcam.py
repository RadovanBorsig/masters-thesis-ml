...
model.fc = nn.Sequential(torch.nn.Dropout(p=0.5), nn.Linear(num_features, len(class_names)))
...
def save_pair(img_path, true_label, idx, fname):
    ...
    ax.imshow(vis)
    ax.axis('off')
    ax.set_title(f'ResNet-50  Pred: {pred_name} ({confidence * 100:.1f}%) {correct}', fontsize=10, fontweight='bold', color=color, pad=6)
    plt.tight_layout(pad=0.3)
