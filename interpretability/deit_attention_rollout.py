...
model = timm.create_model('deit3_base_patch16_224', pretrained=False, num_classes=len(class_names))
...
def save_pair(img_path, true_label, idx, fname):
    ...
    ax.imshow(blended)
    ax.axis('off')
    ax.set_title(f'DeiT  Pred: {pred_name} ({confidence * 100:.1f}%) {correct}', fontsize=10, fontweight='bold', color=color, pad=6)
    plt.tight_layout(pad=0.3)
