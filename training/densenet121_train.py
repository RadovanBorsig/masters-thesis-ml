...
model.classifier = nn.Sequential(nn.Dropout(p=0.5), nn.Linear(num_features, 4))
...
torch.save(model.state_dict(), f"training/models/denseNet121_{EXP_NAME}.pth")
print(f"Final model saved as denseNet121_{EXP_NAME}.pth")
...
plt.tight_layout()
plt.savefig(f"denseNet121_progress_{EXP_NAME}.png", dpi=300, bbox_inches='tight')
plt.show()
